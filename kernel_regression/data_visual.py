...
def main():
    # Load data
    knn_mse = misc.load_csv('error_knn_1000_700.csv')
    reg_mse = misc.load_csv('error_reg_1000_700.csv')
    ...
    #kde.fit(knn[:, None])
    plt.hist(knn, bins = 30,  alpha=0.5, color="red", label='KNN')
    plt.hist(reg, bins = 30, alpha=0.5, color = "blue", label='Regression')
    plt.axvline(x = mean_knn, color = "red")
    plt.axvline(x = mean_reg,  color = "blue")
    plt.legend(loc='upper right')
    plt.title("1000 montecarlo simulations with 700 samples")
    plt.show()
